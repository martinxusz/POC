...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
			...
			initComponent : function() {
				...
				this.addEvents("focus", "blur", "specialkey", "change",
						"invalid", "valid");
				if(this.readOnly && this.getXType()!='combo'){   
					this.fieldClass="x-form-field x-item-readonly x-item-disabled"  
				} 
			},
			getName : function() {
...
Ext.form.NumberField = Ext.extend(Ext.form.TextField, {
	...
	allowSeparator : true,		//以下两个为自定义属性(维护人：叶旭)
	separator : "。",			//
	initEvents : function() {
		Ext.form.NumberField.superclass.initEvents.call(this);
		var b = this.baseChars + "";
		//判断是否允许其他字符 (自定义) start
		if(this.allowSeparator && this.separator){
			b += this.separator
		}
		//判断是否允许其他字符 (自定义) end
		 ...
		 */
		 var keyUp = function(e){
        	if(e.getKey()==110 || e.getKey()==190){
        		Ext.getDom(this.el).value=this.getValue()+"."
        	}
        };
	...
	beforeBlur : function() {
		...
		//字定义替换=========start
		/*
		var cus_rawValue = this.getRawValue();
		if(this.allowSeparator){
			cus_rawValue =  String(cus_rawValue).replace(this.separator, ".")
		}
		//字定义替换=========end
		//var a = this.parseValue(this.getRawValue());
		var a = this.parseValue(cus_rawValue);
		*/
		var a = this.parseValue(this.getRawValue());
...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField, {
	...
	validateValue : function(e) {
		if (e.length == 8 && e.indexOf('-')==-1 && e.indexOf('/')==-1) {
			var convertDate = e.substring(0, 4)
					+ '-' + e.substring(4, 6) + '-'
					+ e.substring(6, 8);
			e = convertDate;
			Ext.form.DateField.superclass.setValue.call(this, e);
			return true
		}
		e = this.formatDate(e);
