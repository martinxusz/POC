...
Ext.onReady(function() {
	...
	var actions = {
		...
		deleteList : new Ext.Action({
					...
					disabled : true,
					handler : function() {
						tree.removeList(tree.getSelectionModel()
								.getSelectedNode());
					}
				}),
		...
		pasteAsTask : new Ext.Action({
					...
					handler : function() {
						if (air.Clipboard.generalClipboard
								.hasFormat(air.ClipboardFormats.TEXT_FORMAT)) {
							var text = air.Clipboard.generalClipboard
									.getData(air.ClipboardFormats.TEXT_FORMAT);
							...
							Ext.Msg
									.alert('Warning',
											'Could not create task. The clipboard is empty.');
						}
					}
				})
	};
	...
	menus.add('File', [actions.newTask, actions.newList, actions.newFolder,
					'-', {
						text : 'Import...',
						handler : function() {
	...
	menus.add('Edit', [actions.pasteAsTask]);
	...
	menus.add('Help', [{
						text : 'About',
						handler : function() {
							Ext.air.NativeWindowManager.getAboutWindow()
									.activate();
						}
					}]);
	...
	var tb = new Ext.Toolbar({
		...
		items : [
				...
				}, '-', actions.deleteTask, actions.complete, actions.active,
				'-', actions.pasteAsTask, '->', {
					xtype : 'switch',
					id : 'filter',
					activeItem : 0,
					...
					listeners : {
						...
						delay : 10
						// delay gives user instant click feedback before
						// filtering tasks
					}
				}, ' ', ' ', ' ']
	});
	...
	var loadList = function(listId) {
		...
		actions.deleteFolder.setDisabled(!node
