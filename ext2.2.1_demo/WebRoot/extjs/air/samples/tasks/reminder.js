...
Ext.onReady(function() {
			...
			win.title = 'Reminder - '
					+ Ext.util.Format.ellipsis(task.data.title, 40);

			bulkUpdate({
						'task-title' : Ext.util.Format.ellipsis(
								task.data.title, 80),
						'task-due' : task.data.dueDate ? task.data.dueDate
								.format('F d, Y') : 'None'
					});
			...
			var snooze = new Ext.Button({
						...
						renderTo : 'btns',
						handler : function() {
							var min = parseInt(Ext.get('snooze-time')
											.getValue(), 10);
