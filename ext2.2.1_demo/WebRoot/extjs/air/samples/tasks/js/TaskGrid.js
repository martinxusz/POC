...
TaskGrid = function() {
	...
	Ext.TaskMgr.start({
				run : function() {
					...
					if (col.groupRenderer.date.getTime() != new Date()
							.clearTime().getTime()) {
						col.groupRenderer = Ext.util.Format
								.createTextDateRenderer();
...
Ext.extend(TaskGrid, Ext.grid.EditorGridPanel, {
	onCellDblClick : function(g, row) {
		clearTimeout(this.autoEditTimer); // allow dbl click without starting
											// edit
	...
	onAutoEditClick : function(e, t) {
		...
		if (row !== false && col !== false) {
			if (this.selModel.isSelected(row) && this.selModel.getCount() === 1) {
				this.autoEditTimer = this.startEditing.defer(300, this, [row,
								col]);
			}
		}
	},
		...
		if (!this.menu) { // create context menu on first right click
			this.menu = new Ext.menu.Menu({
						...
						items : [{
							...
							scope : this,
							handler : function() {
								Ext.each(this.selModel.getSelections(),
										function(r) {
											Ext.air.NativeWindowManager
													.getTaskWindow(r.id);
										});
							}
						}, '-', tx.actions.complete, tx.actions.deleteTask]
					});
		}
	...
	function doBlur() {
		if (editing && !focused) {
			...
			if (!Ext.isEmpty(title)) {
				tx.data.tasks.createTask(title, ntCat.getRawValue(), ntDue
								.getValue());
				ntTitle.setValue('');
				if (userTriggered) { // if they entered to add the task, then
										// go to a new add automatically
					userTriggered = false;
