...
Ext.menu.TreeItem = Ext.extend(Ext.menu.Adapter, {
			constructor : function(config) {
				...
				this.tree.on("render", function(tree) {
							tree.body.swallowEvent(['click', 'keydown',
									'keypress', 'keyup']);
						});
...
Ext.ux.TreeSelector = Ext.extend(Ext.form.TriggerField, {
	initComponent : function() {
		...
		this.tree.getSelectionModel().on('selectionchange', this.onSelection,
				this);
		this.tree.on({
		...
		if (this.menu && this.menu.isVisible()) {
			if (this.tree.body.getHeight() > this.maxHeight) {
				...
				this.restricted = true;
					...
					&& this.tree.body.dom.firstChild.offsetHeight < this.maxHeight) {
	...
	onRender : function() {
		...
		this.menu = new Ext.menu.TreeMenu(Ext.apply(this.menuConfig || {}, {
					tree : this.tree
				}));
		this.menu.render();
...
/*
 * Custom tree keyboard navigation that supports node navigation without
 * selection
 */
Ext.tree.ActivationModel = Ext.extend(Ext.tree.DefaultSelectionModel, {
			select : function(node) {
				return this.activate(Ext.tree.ActivationModel.superclass.select
						.call(this, node));
			},
