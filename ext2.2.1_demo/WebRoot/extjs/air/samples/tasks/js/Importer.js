...
tx.Importer = function() {
	...
	/*
	 * This import function used to be clean and simple. The addition of
	 * asynchronous import and a progress bar changed that quickly. :)
	 */
	function doImport(file, callback) {
		...
		function nextTask(tasks, index, lists, listIndex) {
			if (index < tasks.length) {
				Ext.Msg.updateProgress(getProgress(++visibleIndex), f(
								'{0} of {1}', visibleIndex, taskCount));
