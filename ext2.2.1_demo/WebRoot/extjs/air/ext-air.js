...
Ext.air.NativeWindow = function(config) {
	...
	this.addEvents('close', 'closing', 'move', 'moving', 'resize', 'resizing',
			'displayStateChange', 'displayStateChanging');
...
Ext.extend(Ext.air.NativeWindow, Ext.air.NativeObservable, {
	...
	fullscreen : function(nonInteractive) {
		...
		this.instance.stage.displayState = nonInteractive
				? SDS.FULL_SCREEN
				: SDS.FULL_SCREEN_INTERACTIVE;
	},
...
Ext.sql.SQLiteStore = Ext.extend(Ext.data.Store, {
			constructor : function(config) {
				...
				this.proxy = new Ext.sql.Proxy(conn, config.tableName,
						config.key, this, false);
			}
		});
...
Ext.air.MusicPlayer = Ext.extend(Ext.util.Observable, {
			...
			constructor : function(config) {
				config = config || {};
				Ext.apply(this, config);
				this.addEvents('stop', 'pause', 'play', 'load', 'id3info',
						'complete', 'progress', 'skip');
				Ext.air.MusicPlayer.superclass.constructor.call(this, config);
				this.onSoundFinishedDelegate = this.onSoundFinished
						.createDelegate(this);
				this.onSoundLoadDelegate = this.onSoundLoad
						.createDelegate(this);
				this.onSoundID3LoadDelegate = this.onSoundID3Load
						.createDelegate(this);
				Ext.TaskMgr.start({
			...
			stop : function() {
				...
				if (this.activeSound) {
					this.activeSound.removeEventListener(air.Event.COMPLETE,
							this.onSoundLoadDelegate);
					this.activeSound.removeEventListener(air.Event.ID3,
							this.onSoundID3LoadDelegate);
					this.activeSound.removeEventListener(
							air.Event.SOUND_COMPLETE,
							this.onSoundFinishedDelegate);
				}
			},
			pause : function() {
				if (this.activeChannel) {
			...
			play : function(url) {
				if (url) {
					...
					this.activeSound.addEventListener(air.Event.SOUND_COMPLETE,
							this.onSoundFinishedDelegate);
					this.activeSound.addEventListener(air.Event.COMPLETE,
							this.onSoundLoadDelegate);
					this.activeSound.addEventListener(air.Event.ID3,
							this.onSoundID3LoadDelegate);
			...
			skipTo : function(pos) {
				if (this.activeChannel) {
					...
					this.activeChannel.soundTransform = this.activeTransform;
					this.fireEvent('skip', this.activeChannel,
							this.activeSound, pos);
				}
			},
			...
			onSoundLoad : function(event) {
				if (this.activeSound) {
					...
					this.activeChannel = this.activeSound
							.play(this.pausePosition);
					this.activeChannel.soundTransform = this.activeTransform;
					this
							.fireEvent('load', this.activeChannel,
									this.activeSound);
				}
			},
			...
			notifyProgress : function() {
				if (this.activeChannel && this.activeSound) {
					var playbackPercent = 100
							* (this.activeChannel.position / this.activeSound.length);
					if (playbackPercent > 99.7) {
						this.onSoundFinished();
					} else {
						this.fireEvent('progress', this.activeChannel,
								this.activeSound);
					}
				}
			}
...
Ext.air.Notify = Ext.extend(Ext.air.NativeWindow, {
	...
	constructor : function(config) {
		...
		this.loader.addEventListener(air.Event.COMPLETE,
				this.onCompleteDelegate);
	},
	onComplete : function(event) {
		this.loader.removeEventListener(air.Event.COMPLETE,
				this.onCompleteDelegate);
		this.show(event);
	},
	show : function(event) {
		var h = event.target.window.document.getElementById(this.msgId).clientHeight
				+ this.extraHeight;
			...
			dataFn : function(currNode) {
				...
				if (currNode.attributes.url) {
					currDir = this.directory
							.resolvePath(currNode.attributes.url);
				} else {
					currDir = this.directory;
				}
				var files = [];
				...
				for (i = 0; i < c.length; i++) {
					if (c[i].isDirectory || this.extensionFilter === false
							|| this.extensionFilter === c[i].extension)
						files.push({
									text : c[i].name,
									url : c[i].url,
...
(function		() {
					...
					this.video = new air.Video(this.getInnerWidth(), this
									.getInnerHeight());
					if (this.url) {
