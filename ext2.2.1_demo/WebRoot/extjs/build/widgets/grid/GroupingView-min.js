Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	getGroup : function(a, d, f, h, b, e) {
		var c = f ? f(a, {}, d, h, b, e) : String(a);
		if (c === "") {
	...
	doRender : function(d, h, s, a, q, u) {
		if (h.length < 1) {
			return ""
		}
		var B = this.getGroupField();
		var p = this.cm.findColumnIndex(B);
		...
		var t = this.grid.getGridEl().id;
		var f = this.cm.config[p];
		var b = f.groupRenderer || f.renderer;
		var v = this.showGroupName ? (f.groupName || f.header) + ": " : "";
		var A = [], m, w, x, o;
		for (w = 0, x = h.length; w < x; w++) {
			var l = a + w;
			var n = h[w], e = n.data[B], y = this.getGroup(e, n, b, l, p, s);
			if (!m || m.group != y) {
				o = t + "-gp-" + B + "-" + Ext.util.Format.htmlEncode(y);
				var c = typeof this.state[o] !== "undefined"
						? !this.state[o]
						: this.startCollapsed;
				var k = c ? "x-grid-group-collapsed" : "";
				m = {
					group : y,
					gvalue : e,
					text : v + y,
		...
		for (w = 0, x = A.length; w < x; w++) {
			var y = A[w];
			this.doGroupStart(z, y, d, s, q);
			z[z.length] = Ext.grid.GroupingView.superclass.doRender.call(this,
					d, y.rs, s, y.startRow, q, u);
			this.doGroupEnd(z, y, d, s, q)
		}
		return z.join("")
	},
