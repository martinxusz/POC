 ...
 * BSD License: http://developer.yahoo.net/yui/license.txt version: 2.4.1
 */
...
(function() {
	...
	YAHOO.util.Dom = {
		...
		getXY : function(Q) {
			var R = function(S) {
				if ((S.parentNode === null || S.offsetParent === null || this
		...
		setXY : function(Q, T, S) {
			var R = function(W) {
				...
				var X = [parseInt(this.getStyle(W, "left"), 10),
		...
		getClientRegion : function() {
			var S = B.Dom.getDocumentScrollTop(), R = B.Dom
					.getDocumentScrollLeft(), T = B.Dom.getViewportWidth() + R, Q = B.Dom
					.getViewportHeight()
					+ S;
	...
	var I = function() {
		if (N.documentElement.getBoundingClientRect) {
			return function(R) {
				...
				return [S.left + B.Dom.getDocumentScrollLeft(Q),
						S.top + B.Dom.getDocumentScrollTop(Q)];
			};
		} else {
			return function(S) {
	...
	YAHOO.util.DragDropMgr = function() {
		...
		return {
			...
			fireEvents : function(T, J) {
				...
				var L = YAHOO.util.Event.getPageX(T), K = YAHOO.util.Event
						.getPageY(T), M = new YAHOO.util.Point(L, K), H = V
						.getTargetCoord(M.x, M.y), E = V.getDragEl(), S = new YAHOO.util.Region(
						H.y, H.x + E.offsetWidth, H.y + E.offsetHeight, H.x), G = [], I = [], D = [], U = [], R = [], C = {}, N = [];
			...
			getElWrapper : function(C) {
				...
				if (!B || !B.el) {
					B = this.elementCache[C] = new this.ElementWrapper(YAHOO.util.Dom
							.get(C));
				}
				return B;
			},
