...
(function() {
	...
	Ext.apply(Ext, {
		...
		BLANK_IMAGE_URL : "/ehs/extjs/resources/images/default/s.gif",
		emptyFn : function() {
		},
...
/*
 * 重写window.open方法 主要设置一个窗口只能打开一次(必须关闭本窗口,才能再次打开) author:吴寅寅 date:2010-06-23
 */
windowOpenView = function(url) {
	var windowName = url;
	if (url.indexOf('.jsp?') != -1) {
		windowName = url.substring(0, url.indexOf('.jsp?'));
	} else if (url.indexOf('.jsp') != -1) {
		windowName = url.substring(0, url.indexOf('.jsp'));
	} else if (url.indexOf('.do?method=') != -1) {
		windowName = url.replace('.do?method=', '');
		if (windowName.indexOf('&') != -1) {
			windowName = windowName.substring(0, windowName.indexOf('&'));
		}
	} else if (url.indexOf('&') != -1) {
		windowName = url.substring(0, url.indexOf('&'));
	} else if (url.indexOf('.do') != -1) {
		windowName = url.substring(0, url.indexOf('.do'));
	}
	if (windowName.indexOf('/') != -1)
		windowName = windowName.replace(new RegExp('/', 'g'), '_');
	var features = typeof(arguments[2]) == "undefined" ? "" : arguments[2]
			.toString();
...
document.onkeydown = function(event) {
 // 如果按下的是退格键
  ...
  function resetForm(){
   ...
   for(var ii=8;ii<inputObj.length;ii++){
     // alert(inputObj[ii].type+"="+inputObj[ii].value); 
      if(inputObj[ii].type=="text"&&inputObj[ii].type!="hidden"
         &&inputObj[ii].type!="submit"&&inputObj[ii].disabled!=true
         &&inputObj[ii].type!="button"&&inputObj[ii].readonly!=true){
         inputObj[ii].value=0;
      } 
   }
  }
